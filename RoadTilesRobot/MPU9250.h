...
#include "libs/MPU9250/MPU9250RegisterMap.h"
#include "libs/MPU9250/QuaternionFilter.h"
...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), WireType& w = Wire) {
        ...
        if (!b_use_AK8963) {
            if (b_verbose) {
                Serial.println();
                Serial.println("AK8963 is DISABLED by user! (do you have a fake MPU9250?)");
            }
            // only the (default) MADGWICK filter uses AK8963
            selectFilter(QuatFilterSel::MAHONY); //NONE, MADGWICK, MAHONY,
            //setFilterKp(0.5);
        }
    ...
    void set_use_AK8963(const bool b) {
        b_use_AK8963 = b;
    }
    ...
    void setFilterKp(float Kp) {
        quat_filter.set_Kp(Kp);
    }
...
private:
    void write_byte_eeprom(int address, byte value) {
        EEPROM.put(address, value);
    }
    void write_float_eeprom(int address, float value) {
        EEPROM.put(address, value);
    }
    ...
    float read_float_eeprom(int address) {
        float valueIn = 0;
        EEPROM.get(address, valueIn);
        return valueIn;
    }
...
public:
    void clearCalibration() {
        write_byte_eeprom(EEP_CALIB_FLAG, 0);
    }
    bool isCalibrated() {
        return (read_byte_eeprom(EEP_CALIB_FLAG) == 0x01);
    }
    void saveCalibration() {
        if (b_verbose) {
          Serial.println("Write calibrated parameters to EEPROM");
        }
        write_byte_eeprom(EEP_CALIB_FLAG, 1);
        write_float_eeprom(EEP_ACC_BIAS + 0, getAccBias(0));
        write_float_eeprom(EEP_ACC_BIAS + 4, getAccBias(1));
        write_float_eeprom(EEP_ACC_BIAS + 8, getAccBias(2));
        write_float_eeprom(EEP_GYRO_BIAS + 0, getGyroBias(0));
        write_float_eeprom(EEP_GYRO_BIAS + 4, getGyroBias(1));
        write_float_eeprom(EEP_GYRO_BIAS + 8, getGyroBias(2));
        if (b_use_AK8963) {
            write_float_eeprom(EEP_MAG_BIAS + 0, getMagBias(0));
            write_float_eeprom(EEP_MAG_BIAS + 4, getMagBias(1));
            write_float_eeprom(EEP_MAG_BIAS + 8, getMagBias(2));
            write_float_eeprom(EEP_MAG_SCALE + 0, getMagScale(0));
            write_float_eeprom(EEP_MAG_SCALE + 4, getMagScale(1));
            write_float_eeprom(EEP_MAG_SCALE + 8, getMagScale(2));
        }
    ...
    void loadCalibration() {
        ...
        if (isCalibrated()) {
            ...
            setAccBias(
                read_float_eeprom(EEP_ACC_BIAS + 0),
                read_float_eeprom(EEP_ACC_BIAS + 4),
                read_float_eeprom(EEP_ACC_BIAS + 8));
            setGyroBias(
                read_float_eeprom(EEP_GYRO_BIAS + 0),
                read_float_eeprom(EEP_GYRO_BIAS + 4),
                read_float_eeprom(EEP_GYRO_BIAS + 8));
            if (!b_use_AK8963) {
                setMagBias(
                    read_float_eeprom(EEP_MAG_BIAS + 0),
                    read_float_eeprom(EEP_MAG_BIAS + 4),
                    read_float_eeprom(EEP_MAG_BIAS + 8));
                setMagScale(
                    read_float_eeprom(EEP_MAG_SCALE + 0),
                    read_float_eeprom(EEP_MAG_SCALE + 4),
                    read_float_eeprom(EEP_MAG_SCALE + 8));
            }
        } else {
            if (b_verbose) {
    ...
    void print_calibration() {
        ...
        Serial.print(getAccBiasX() * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.print(", ");
        Serial.print(getAccBiasY() * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.print(", ");
        Serial.print(getAccBiasZ() * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.println();
        Serial.println("gyro bias [deg/s]: ");
        Serial.print(getGyroBiasX() / (float)CALIB_GYRO_SENSITIVITY);
        Serial.print(", ");
        Serial.print(getGyroBiasY() / (float)CALIB_GYRO_SENSITIVITY);
        Serial.print(", ");
        Serial.print(getGyroBiasZ() / (float)CALIB_GYRO_SENSITIVITY);
        Serial.println();
        if (b_use_AK8963) {
