...
private:
    template <typename T> T* quat_normalize(T* q) {
        T len = q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3];
        T norm = 1.0 / sqrt(len);
        q[0] *= norm;
        q[1] *= norm;
        q[2] *= norm;
        q[3] *= norm;
        //if (len == 0.0) return (T*)NULL;  // handle NaN
        return q;
    }
    template <typename T> T*vec3(T x, T y, T z) {
        T v[3] = {x, y, z};
        return v;
    }
...
public:
    ...
    void no_filter(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q) {
        ...
        float mul = 0.5f * deltaT;
        q[0] += (-q1 * gx - q2 * gy - q3 * gz) * mul;
        q[1] +=  (q0 * gx + q2 * gz - q3 * gy) * mul;
        q[2] +=  (q0 * gy - q1 * gz + q3 * gx) * mul;
        q[3] +=  (q0 * gz + q1 * gy - q2 * gx) * mul;
        quat_normalize(q);
    }
